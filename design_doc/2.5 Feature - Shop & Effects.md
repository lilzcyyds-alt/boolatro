...
### 1) EffectTrigger (触发器)
定义了效果触发的时机：
- `onRoundStart`: 盲注开始。**（已实现）**
- `onProofSubmitted`: 提交证明时。**（已实现）**
- `onRoundEnd`: 盲注结束结算。**（计划中 - 枚举已定义但未实际调用）**
- `onShopEnter/onShopExit`: 商店交互。**（计划中 - 枚举已定义但未实际调用）**
...
卡牌通过 `assets/config/cards.json` 进行配置。详细字段说明请参考 [3.1 Cards.md](./3.1%20Cards.md)。

`CardSystem` 负责加载此配置并驱动商店库存生成。金钱结算（Cashout）现在直接根据 `blinds.json` 中配置的 `reward` 数值进行发放。
