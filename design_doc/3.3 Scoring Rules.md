...
本表是实现的单一事实来源（Single Source of Truth）。
...
| &elim | 30 | 2 | 与消去 |
| &intro | 20 | 1 | 与引入 |
| ~elim | 40 | 2 | 非消去（双重否定消去） |
| ~intro | 20 | 1 | 非引入（双重否定引入） |
...
4. **分数累积性**：在单个 Blind 内，玩家每次提交的分数会持续累积到 `blindScore` 中。只有在击败当前 Blind 并结算进入 Cashout 后，下一场 Blind 开始时分数才会清零。
