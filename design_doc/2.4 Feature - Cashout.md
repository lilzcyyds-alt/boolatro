...
### 1. 收益构成
- **Base Payout**: 完成盲注的基础奖励（目前简单设定为分数除以 10）。
- **Hand Bonus**: 每一剩余的手牌 (`handsRemaining`) 提供的额外金钱。**（计划中，当前版本未包含）**
- **Interest**: 基于当前持有的金钱，每 5 元产生 1 元利息（最高 5 元）。**（计划中，当前版本未包含）**

### 2. 特殊加成
- 某些特殊卡牌可能在结算时触发 `onRoundEnd`，提供额外的金钱或资源加成。**（计划中 - 触发器已定义但当前版本未实际调用）**
