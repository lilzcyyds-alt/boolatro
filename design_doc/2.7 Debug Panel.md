...
### 2. 界面布局 (UI Layout)
面板采用半透明深色背景，支持 **展开/折叠** 状态，确保不触发调试功能时最小化对画面的干扰。

- **折叠状态 (Collapsed)**: 显示为一个带 `bug_report` 图标的小按钮 (`Material` + `InkWell`)。
- **展开状态 (Expanded)**: 显示完整调试列表，右上角有关闭按钮。
- **背景**: `Colors.black54`。
- **边框**: `Colors.white24` (仅展开态)。
